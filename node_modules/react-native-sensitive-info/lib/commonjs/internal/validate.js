...
exports.MAX_VALUE_BYTES = exports.MAX_SERVICE_BYTES = exports.MAX_KEY_BYTES = void 0;
exports.validateKey = validateKey;
exports.validateService = validateService;
exports.validateValue = validateValue;
var _errors = require("../errors.js");
/** Maximum byte length accepted for an entry key. */
const MAX_KEY_BYTES = exports.MAX_KEY_BYTES = 1024;

/** Maximum byte length accepted for a service name. */
const MAX_SERVICE_BYTES = exports.MAX_SERVICE_BYTES = 1024;

/** Maximum byte length accepted for a stored value (UTF-8 encoded). Default: 1 MiB. */
const MAX_VALUE_BYTES = exports.MAX_VALUE_BYTES = 1024 * 1024;
...
function validateKey(key) {
  if (typeof key !== 'string') {
    throw new _errors.InvalidArgumentError(`Expected key to be a string, received ${typeof key}.`, {
      argument: 'key'
    });
  }
  if (key.trim().length === 0) {
    throw new _errors.InvalidArgumentError('Expected key to be a non-empty string.', {
      argument: 'key'
    });
  }
  const size = utf8ByteLength(key);
  if (size > MAX_KEY_BYTES) {
    throw new _errors.InvalidArgumentError(`Expected key to be at most ${MAX_KEY_BYTES} bytes (UTF-8), received ${size}.`, {
      argument: 'key'
    });
  }
}
 ...
 * @internal
 */
function validateService(options) {
  const service = options?.service;
  if (service === undefined) return;
  if (typeof service !== 'string') {
    throw new _errors.InvalidArgumentError(`Expected options.service to be a string, received ${typeof service}.`, {
      argument: 'service'
    });
  }
  if (service.trim().length === 0) {
    throw new _errors.InvalidArgumentError('Expected options.service to be a non-empty string.', {
      argument: 'service'
    });
  }
  const size = utf8ByteLength(service);
  if (size > MAX_SERVICE_BYTES) {
    throw new _errors.InvalidArgumentError(`Expected options.service to be at most ${MAX_SERVICE_BYTES} bytes (UTF-8), received ${size}.`, {
      argument: 'service'
    });
  }
}
...
function validateValue(value) {
  if (typeof value !== 'string') {
    throw new _errors.InvalidArgumentError(`Expected value to be a string, received ${typeof value}.`, {
      argument: 'value'
    });
  }
  const size = utf8ByteLength(value);
  if (size > MAX_VALUE_BYTES) {
    throw new _errors.InvalidArgumentError(`Expected value to be at most ${MAX_VALUE_BYTES} bytes (UTF-8), received ${size}.`, {
      argument: 'value'
    });
  }
}
