...
import { isNotFoundError, toSensitiveInfoError } from "../errors.js";
import getNativeInstance from "../internal/native.js";
import { normalizeOptions, normalizePromptedReadOptions, normalizeStorageScopeOptions } from "../internal/options.js";
import { validateKey, validateService, validateValue } from "../internal/validate.js";
...
export async function getItem(key, options) {
  ...
  const payload = {
    key,
    includeValue,
    ...(includeValue ? normalizePromptedReadOptions(options) : normalizeStorageScopeOptions(options))
  };
  try {
...
export async function getAllItems(options) {
  ...
  const payload = {
    ...
    ...(options?.includeValues === true ? normalizePromptedReadOptions(options) : normalizeStorageScopeOptions(options))
  };
  try {
...
export async function rotateKeys(options) {
  ...
  const payload = {
    ...
    ...(options?.reEncryptEagerly === true ? normalizePromptedReadOptions(options) : normalizeStorageScopeOptions(options))
  };
  try {
 ...
 * @see {@link rotateKeys}
 */
export async function getKeyVersion(options) {
  validateService(options);
  const native = getNativeInstance();
  try {
