...
export function validateKey(key) {
  if (typeof key !== 'string') {
    throw new InvalidArgumentError(`Expected key to be a string, received ${typeof key}.`, {
      argument: 'key'
    });
  }
  ...
  if (size > MAX_KEY_BYTES) {
    throw new InvalidArgumentError(`Expected key to be at most ${MAX_KEY_BYTES} bytes (UTF-8), received ${size}.`, {
      argument: 'key'
    });
  }
}
 ...
 * @internal
 */
export function validateService(options) {
  const service = options?.service;
  if (service === undefined) return;
  if (typeof service !== 'string') {
    throw new InvalidArgumentError(`Expected options.service to be a string, received ${typeof service}.`, {
      argument: 'service'
    });
  }
  if (service.trim().length === 0) {
    throw new InvalidArgumentError('Expected options.service to be a non-empty string.', {
      argument: 'service'
    });
  }
  const size = utf8ByteLength(service);
  if (size > MAX_SERVICE_BYTES) {
    throw new InvalidArgumentError(`Expected options.service to be at most ${MAX_SERVICE_BYTES} bytes (UTF-8), received ${size}.`, {
      argument: 'service'
    });
  }
}
...
export function validateValue(value) {
  if (typeof value !== 'string') {
    throw new InvalidArgumentError(`Expected value to be a string, received ${typeof value}.`, {
      argument: 'value'
    });
  }
  const size = utf8ByteLength(value);
  if (size > MAX_VALUE_BYTES) {
    throw new InvalidArgumentError(`Expected value to be at most ${MAX_VALUE_BYTES} bytes (UTF-8), received ${size}.`, {
      argument: 'value'
    });
  }
}
