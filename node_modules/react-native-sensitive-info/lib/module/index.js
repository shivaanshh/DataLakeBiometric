...
import { DEFAULT_ACCESS_CONTROL, DEFAULT_KEYCHAIN_SERVICE, ErrorCode } from "./types.js";

// ============================================================================
// RE-EXPORT UNIFIED TYPES (from src/types.ts)
// ============================================================================

export { SensitiveInfoError, ErrorCode, isSensitiveInfoError, SUPPORTED_PLATFORMS, MIN_VERSIONS, DEFAULT_ACCESS_CONTROL, DEFAULT_KEYCHAIN_SERVICE, MAX_VALUE_LENGTH } from "./types.js";
...
async function invokeNative(nativeModule, method, args) {
...
function fallbackSetItem(service, key, value, accessControl) {
...
function fallbackGetItem(service, key) {
  const storageKey = createStorageKey(service, key);
  return mockStorage[storageKey] ?? null;
}
function fallbackHasItem(service, key) {
  const storageKey = createStorageKey(service, key);
  return storageKey in mockStorage;
}
function fallbackDeleteItem(service, key) {
  const storageKey = createStorageKey(service, key);
  delete mockStorage[storageKey];
}
function fallbackGetAllItems(service) {
  const prefix = createStorageKey(service, '');
  return Object.keys(mockStorage).filter(key => key.startsWith(prefix)).map(key => key.substring(prefix.length));
}
function fallbackClearService(service) {
  const prefix = createStorageKey(service, '');
 ...
 * @private
 */
function createStorageKey(service, key) {
  return `${service}:${key}`;
}
...
export async function setItem(key, value, options) {
  ...
  try {
    const {
      didInvoke,
      result
    } = await invokeNative(nativeModule, 'setItem', [key, value, {
      service,
      ...options
    }]);
    if (didInvoke) {
...
export async function getItem(key, options) {
  ...
  try {
    const {
      didInvoke,
      result
    } = await invokeNative(nativeModule, 'getItem', [key, {
      service,
      ...options
    }]);
    if (didInvoke) {
 ...
 * ```
 */
export async function hasItem(key, options) {
  if (!key) {
    throw new Error('Key is required');
  }
  const service = resolveService(options);
  ...
  try {
    const {
      didInvoke,
      result
    } = await invokeNative(nativeModule, 'hasItem', [key, {
      service
    }]);
    if (didInvoke) {
      return result;
...
export async function deleteItem(key, options) {
  if (!key) {
    throw new Error('Key is required');
  }
  const service = resolveService(options);
  ...
  try {
    const {
      didInvoke
    } = await invokeNative(nativeModule, 'deleteItem', [key, {
      service
    }]);
    if (didInvoke) {
      return;
...
export async function getAllItems(options) {
  ...
  try {
    const {
      didInvoke,
      result
    } = await invokeNative(nativeModule, 'getAllItems', [{
      service
    }]);
    if (didInvoke) {
      return result ?? [];
 ...
 * ```
 */
export async function clearService(options) {
  const service = resolveService(options);
  const nativeModule = getNativeModule();
  try {
    const {
      didInvoke
    } = await invokeNative(nativeModule, 'clearService', [{
      service
    }]);
    if (didInvoke) {
      return;
...
export async function getSupportedSecurityLevels() {
  ...
  try {
    const {
      didInvoke,
      result
    } = await invokeNative(nativeModule, 'getSupportedSecurityLevels', []);
    if (didInvoke && result) {
      return result;
    }
